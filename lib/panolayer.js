...
PanoramioLayerCallback.prototype.createMarker = function(photo, baseIcon) {
  ...
  var thingy = {type: 'pano', html: html, marker: marker};
  
  GEvent.addListener(marker, "click",           function() { Viewer.open(thingy); });
  GEvent.addListener(marker, "infowindowclose", function() { Viewer.close(thingy); });
