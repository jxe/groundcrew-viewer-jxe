...
function ev(city_id, venture_uuid, item_id, created_at, atype, msg, x){
  var annc_tag = "Annc__" + venture_uuid + item_id + created_at;
  x = x || {};
  x['msg'] = msg || x['msg'];
  
  return Anncs.add(annc_tag, {
    annc_tag: annc_tag,
    item_tag: "Person__" + item_id,
    created_at: created_at,
    atype: atype,
    actor_tag: "Person__" + item_id,
    re: "Op__v" + venture_uuid,
    atags: atype,
    city_id: city_id
  }, x);
}
...
Anncs = new Resource('Annc', {
  ...
  enhancer: function(event) {
    if (event.re && event.re.resource()) event.re.resource().children.push(event);
