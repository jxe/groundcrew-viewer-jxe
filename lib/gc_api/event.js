...
function ev(city_id, venture_uuid, item_id, created_at, atype, msg, x){
  x = x || {};
  var annc_tag = "Annc__" + (x.uuid || venture_uuid + item_id + created_at);
  ...
  if (atype == "chat") {
    Chats.push({ 
      actor_title: x.actor_title, 
      actor_tag: "Person__" + item_id, 
      what: x.msg, 
      when: created_at });
  }
  ...
  return Anncs.add(annc_tag, {
    annc_tag: annc_tag,
    item_tag: item_id && "Person__" + item_id,
    created_at: created_at,
    atype: atype,
    actor_tag: item_id && "Person__" + item_id,
    re: venture_uuid && "Op__" + venture_uuid,
    atags: atype,
    city_id: city_id
  }, x);
}
...
Event = {
  ...
  post: function(data, callback) {
    ...
    return $.post("/api/event", data, function(result){
      x = eval(result);
      callback && callback(x);
    });
  },
