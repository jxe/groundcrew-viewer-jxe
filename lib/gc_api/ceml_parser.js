...
_CEML = {
  ...
  parse: function(block) {
    ...
    // console.log(block);
    _CEML.context(null, {}, block);
  },
  
  eval: function(block, context) {
    $.each(_CEML.lines(block, true), function(){
      _CEML.eval_line(this, context);
    });
  },
  ...
  eval_line: function(line, context) {
    ...
    if (_CEML[cmd]) {
      try {
        _CEML[cmd](args, context, block, parens);
      } catch(e) {
        App.report_error('error during ceml run on line: ' + 
          cmd + ' ' +  args.join(' ') + ' ' + block, e);
      }
  ...
  context: function(args, context, block) {
    _CEML.eval(block, _CEML.child_context(context));
  },
  
  child_context: function(context) {
    return $.extend({
      generic_roles: { 'all': _CEML.new_role() }
    }, context);
  },
  ...
  role: function(rolename, context) {
    if (context.offering) {
      ...
      if (!x) x = context.generic_roles[rolename] = _CEML.new_role();
      return x;
    }
  },
  
  expand_roles: function(rolespec, context) {
    if (rolespec == 'all') return _CEML.all_roles(context);
    return [_CEML.role(rolespec, context)];
  },
  
  offering: function(args, context, block, paren) {
    context = _CEML.child_context(context);
    ...
    $.each(roles, function(){
      ...
      var count = word[0];
      var rolename = word[1];
      var role = _CEML.new_role(rolename, context);
      context.offering.all_roles.push(role);
      context.offering.roles[rolename] = role;
      Rolebook[args[0] + '/' + rolename] = role;
    });
    
    _CEML.eval(block, context);
    Idea.create(context.offering.name, block, context.offering);
  },
  ...
  everyone: function(args, context, block) {
    _CEML.every(['all'], context, block);
  },
  
  all: function(args, context, block) {
    _CEML.every(args, context, block);
  },
  
  every: function(args, context, block) {
    $.each(_CEML.expand_roles(args[0], context), function(){
      var role = this;
      $.each(_CEML.lines(block), function(){
        var line = this;
        if (/^\s*$/.test(line)) return;
        var functions = [];
        ...
        if (functions.length == 0) {
          // autodetect function
          functions = _CEML.snippet_matches(line);
        }
...
//     _idea(args.first, Number(args.rank || 20), args.roles[0].invite['for'], args.roles[0].features, args);
//   },
