...
Item = {
  ...
  calculated_fields: {
    ...
    availability_status: function(item){
      ...
      var recently = !comm[1] || Date.within(comm[1], 60 * 60);
      
      if (recently) {
        var most_recent_answer = item.answers && $values(item.answers).sort_by(function(x){ return -x[1]; })[0];
      ...
      var recent_content = (item.update || '') + (item.msg || '');
      if (recent_content.contains('?')) return "neglected";
