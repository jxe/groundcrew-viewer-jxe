...
Item = {
  ...
  calculated_fields: [
    ...
    ['fab_state', function(item){
      ...
      var last_answer_and_ts = $values(item.answers).sort_by(function(x){ return -x[1]; });
      var most_recent_answer = Date.within(last_answer_and_ts[1], 24*60*60) && last_answer_and_ts[0];
    ...
    ['recent_dialogue', function(item){
      // some backward compat fixes
      if (!item.latch || !item.comm) return [];
      var comm  = item.comm.split(' ');
      var latch = item.latch.split(' ');
