...
function item(city, tag, title, thumb_url, lat, lng, atags, latch, comm, req, x){
  ...
  var via_sys = Agents.via_sys(tag, comm);
  if (!Agents.reachable(tag, via_sys)) return null;
...
Persons = Agents = new Resource('Person', {
  ...
  init_manual: function() {
    // strip out unreachable agents
    $.each(Agents.everything() || [], function(i, item) {
      item.via_sys = Agents.via_sys(item.id, item.comm);
      if (!Agents.reachable(item.id, item.via_sys)) Agents.remove(item.id);
    });
  },
  via_sys: function(tag, comm) {
  ...
  reachable: function(tag, via_sys) {
    return (tag.resource_type() != 'Person' || via_sys.length > 0);
  },
