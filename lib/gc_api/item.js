...
Item = {
  ...
  calculated_fields: [
    ...
    ['fab_state', function(item){
      ...
      var our_court   = (last_ts == they_said_ts);
      var their_court = (last_ts == we_said_ts);
      ...
      if (our_court) {
        ...
        if (have_question || have_problem) {
          if (!within_four_hrs)  return "addressable";
          if (!within_three_min) return "neglected";
          return "concerns";
        }

        // TODO: most recent answer should only be set if it was really recent
      ...
      if (their_court) {
        ...
        if (!Date.within(they_said_ts, 30 * 60)) return "unresponsive";
        if (latch.contains("unlatched")) return 'in_dialogue';
        
        // TODO: how to tell pending from assigned?
        return "assigned";
      }
