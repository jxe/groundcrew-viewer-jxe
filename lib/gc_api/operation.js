...
Operation = {
  ...
  group_assign: function(agents, what, callback) {
    console.log(agents);
    var op = Operation.create(agents[0], "was assigned to \""+what+"\"", 'assignment group', what);
    callback(op);
  ...
  assign: function(person, assignment, callback) {
    var op = Operation.create(person, "was assigned to \""+assignment+"\"", 'assignment individual', assignment);
    callback(op);
