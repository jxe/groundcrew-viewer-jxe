  ...
  init: function() {
    ...
    $.ajaxSetup({
      error: function(req, textStatus, errorThrown){
        if (req.status == 0) {
          Notifier.error(null, "You are offline.");
          Ajax.schedule_autoload(30 * 1000);
        }
        if (req.status == 400) {
          Notifier.error(null, req.responseText);
          return;
        }
        if (req.status == 401) {
          if (demo) return Notifier.error(null, "That action is not supported in demo mode.");
        ...
        if (req.responseText) {
          ...
          Notifier.error(null, "Oops!  It looks like your connection is having problems. "+
