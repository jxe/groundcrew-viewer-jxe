...
Demo = {
  ...
  w_delay: function(f, delay_min, delay_max) {
    if (delay_min == -1) return f();
    if (delay_min == null) delay_min = 1000;
    if (delay_max == null) delay_max = delay_min * 2;
    var delay = delay_min + Math.rand(delay_max - delay_min);
    setTimeout(f, delay);
  },
  ...
  random_agent: function(city) {
    if (!city) agents = Agents.here(200);
    else agents = Agents.in_city(city).slice(0, 200);
  ...
  yesno_answers: [
    {a: "yes", evtype: 'answered'}, {a: "yes, that sounds great", evtype: 'answered_yes'},
    {a: "no", evtype: 'answered'}, {a: "no, sorry", evtype: 'answered_no'},
    {a: "sure", evtype: 'answered'}, {a: "I'm not sure", evtype: 'answered'}, {a: "Maybe", evtype: 'answered'}
  ],

  get_answer: function(question) {
    if (question.yesno_question()) {
      return Demo.yesno_answers[Math.rand(Demo.yesno_answers.length)];
    }
    return { a: "I'm answering your question", evtype: 'answered' };
  },
  ...
  create_rcv_event: function(atype, person, op, changes, json_etc, delay_min, delay_max) {
    return Demo.w_delay(function(){
      ...
      ev(city_id, op_id, person, now, atype, json_etc["msg"], json_etc, null, person);
    }, delay_min, delay_max);
  },
  ...
  post_event: function(data) {
    return ev(This.city, data.venture, window.authority, Date.unix(), data.type, data.msg, {
      actor_name: window.user_name || 'You'
    });
  },

  // operations

  create_op: function(site, vtype, body) {
    var op_id = 'v' + StreamLoader.uuid();
    var city_id = site.resource().city_id;
    return operation(city_id, op_id, body, vtype, null, null, null, null, site, null,
      window.authority, window.user_name || 'You', Date.unix(), {});
  },

  assign: function(agents, assignment, callback) {
    var op = Demo.create_op(agents[0], '', assignment);
  ...
  invite: function(agents, landmark, title, assignment, callback) {
    var op = Demo.create_op(landmark, 'invite', title);
    go('@' + most_recent_op.id);
    callback && callback(op);
  ...
  question: function(question, agents, site) {
    site = site || agents[0];
    console.log(question + " asked of " + agents);
    var op = Demo.create_op(site, 'question', "Question: " + question);
    go('@' + most_recent_op.id);

    $.each(agents, function(i, agent){
      Demo.offer(agent, op);
      answer = Demo.get_answer(question);
      answers = agent.resource().answers;
      answers[question] = [answer.a, Date.unix() + 30];
      Demo.create_rcv_event(answer.evtype || 'answered', agent, op,
        {latch: "unlatched", msg: answer.a, answers: answers}, {msg: answer.a},
        null, 30*1000);
    });
  },
...
function demo_op(city, mission, reports){
  var op_id = "vx" + StreamLoader.uuid();
  var agents = Agents.in_city(city).slice(0, reports.length * 3);
  var op = operation(city,op_id,mission,"group",null,null,null,null,agents.map('.id').join(' '),null,"pm13ffa328cbf","Organizer",1292627039,{},null);
  
  $.each(reports, function(){
    var agent = agents.pop();
    var report = this;
    if (!agent) return;
    console.log('creating report with ' + report + ', ' + op.id + ', ' + agent.id);
    // Demo.create_rcv_event('accepted', agent.id, op,
      // {latch: "latched 1-10000 " + op.id + " " + This.user.tag, msg: "ok"}, null, -1);
    Demo.create_rcv_event('reported', agent.id, op, {msg: report}, {msg: report}, -1);
  });
  
  $.each(agents, function(){
    var agent = this;
    if (Math.random() < 0.25) {
      Demo.create_rcv_event('declined', agent.id, op, {}, null, -1);
    } else {
      Demo.create_rcv_event('accepted', agent.id, op,
        {latch: "latched 1-10000 " + op.id + " " + This.user.tag, msg: "ok"}, null, -1);
    }
