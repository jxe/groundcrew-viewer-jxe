...
Demo = {
  ...
  random_report: function(agent) {
    ...
    Demo.report(agent.id, "it's snowing!", null, 0);
    return true;
  },
  ...
  assign: function(agents, assignment, callback) {
    ...
    $.each(agents, function(i, agent){
      ...
      if (Math.random() < 0.33) delay = Demo.report(agent, "I'm reporting in", op, delay + 5*1000);
  ...
  message: function(agents, msg, callback)
  {
    callback && callback(op);
    $.each(agents, function(i, agent){
      if (Math.random() < 0.33) Demo.report(agent, "I don't understand. What?", op, 5*1000);
    });
  },
