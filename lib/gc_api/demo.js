...
Demo = {
  ...
  assign: function(agents, assignment, callback) {
    ...
    $.each(agents, function(i, agent){
      ...
      Demo.create_rcv_event('accepted', agent, op,
        {latch: "latched 1-10000 " + op.id + " " + This.user.tag, msg: "ok"}, null, 2000, 8000);
      Demo.create_rcv_event('completed', agent, op, {latch: "unlatched", msg: "done"}, null, 8100);
    });
  },
  ...
  invite: function(landmark, title, assignment, callback) {
    ...
    $.each(Agents.in_city(op.city_id).slice(0, 10), function(i, agent){
      ...
      Demo.create_rcv_event('accepted', agent.id, op,
        {latch: "latched 1-10000 " + op.id + " " + This.user.tag, msg: "ok"}, null, 2000, 8000);
      Demo.create_rcv_event('completed', agent.id, op, {latch: "unlatched", msg: "done"}, null, 8100);
    });
  },
  ...
  question: function(question, agents) {
    ...
    $.each(agents, function(i, agent){
      Demo.offer(agent, op);
      answer = "wow, you're a real cool cat";
      answers = agent.resource().answers;
      answers[question] = [answer, Date.unix() + 2000];
