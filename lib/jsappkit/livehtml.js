...
LiveHTML = {
  ...
  init: function() {
    ...
    $('a[href],img[href],dl[href],li[href],div[href],h2[href]').live('click', function(){
      ...
      if (!href || href[0] != "#") window.location = href;
      go(href.slice(1));
      return false;
    });
  },
...
$.fn.app_paint = function(){
  ...
  this.find('form').enable().unbind('submit').submit(function(){
    ...
    $(this).disable();
    $(this).find('input').each(function(){ this.value = null; });
    if (this.id) {
      dispatch(this.id + "_submitted", data, This, this);      
    }
  ...
  this.find('input[hint],textarea[hint]').each(function(){
    ...
    if (hint[0] == '#') hint = dispatch(hint.slice(1));
    // if (!self.val()) 
