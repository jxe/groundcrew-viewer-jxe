...
function go(url, form_data) {
  ...
  if (url[0] == '@') {
    ...
    else return go('item=' + url.slice(1));
  }
  ...
  This.changed = {};
  This.prev_state = $.extend({}, This);
  $.each(url.split(';'), function(){
    var part = this.split('=');
    set(part[0], part[1]);
  });
  App.update(This.changed);
};
