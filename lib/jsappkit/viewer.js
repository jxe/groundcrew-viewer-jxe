...
Viewer = {
  ...
  set_state: function(url, form_data) {
    // adjust url
    if (url == '#' || url == '') return;
    ...
    if (!App.modes[app_name]) { alert('invalid url: ' + url); return; }
    if (app_name != Viewer.current_app_name) {
      $('body').removeClass(Viewer.current_app_name + "_app").addClass(app_name + "_app");
      App.modes[app_name].state = { first: true, app: app_name, agents: Viewer.current_app.state.agents };
      Viewer.current_app_name = app_name;
    }
    var app = Viewer.current_app = App.modes[app_name];
  ...
  go: function(url, form_data) {
    Viewer.set_state(url, form_data);
    Viewer.render_state();
  },
  ...
  render_state: function() {
    ...
    // clean up
    $('#breadcrumbs').html(Viewer.breadcrumbs()).val(Viewer.loc);
    delete state.first;
  },
