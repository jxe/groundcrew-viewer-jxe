...
Map = {
  ...
  latlng_open: function(latlng, content, reopen) {
    ...
    if (reopen && latlng == Map.open_site && !GM.getInfoWindow().isHidden()) {
      // GM.updateCurrentTab(function(tab){ tab.contentElem = content; });
      GM.updateInfoWindow([new GInfoWindowTab('', content)]);
      return;
    }
    ...
    GM.openInfoWindow(latlng, content, { noCloseOnClick: true });
  },
  ...
  establish: function(){
    ...
    var bRight = new GControlPosition(G_ANCHOR_BOTTOM_RIGHT, new GSize(5,5));
    var bRightSnug = new GControlPosition(G_ANCHOR_BOTTOM_RIGHT, new GSize(5,5));
    var tRight = new GControlPosition(G_ANCHOR_TOP_RIGHT, new GSize(8,8));
    var tLeftSnug = new GControlPosition(G_ANCHOR_TOP_LEFT, new GSize(5,8));
    var bLeftFurther = new GControlPosition(G_ANCHOR_BOTTOM_LEFT, new GSize(160,5));
    GM.addControl(new GSmallZoomControl(), bRight);
    // GM.addControl(new GMenuMapTypeControl(), tRight);
