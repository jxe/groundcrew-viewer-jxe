...
Map = {  
  ...
  hide_layer: function(lname, clear) {
    ...
    if (!layer) return;
    if (clear) { layer.empty = true; layer.mgr && layer.mgr.clearMarkers(); }
    layer.mgr && layer.mgr.hide();
    layer.unshown = true;
  },
  
  show_layer: function(lname) {
    var markers;
    var layer = Map.layers[lname];
    ...
    if (layer.markers && layer.markers.length > 1) {
      Map.set_bounds_from_lat_lngs(layer.markers);
    } else if (layer.markers && layer.markers.length == 1) {
    ...
    if (!isNaN(map.getZoom())) {
      ...
      if (!layer.markers) alert('no markers for zoom: ' + lname);
    }
    if (layer.empty && layer.mgr && layer.markers.length) {
      layer.mgr.addMarkers(layer.markers, 0);
      $.each(layer.markers, function(){ this.mgr = layer.mgr; });
      layer.empty = false;
    }

    if (layer.unshown && layer.mgr) {
  ...
  add_to_layer: function(lname, markers) {
    ...
    layer.mgr && layer.mgr.addMarkers(markers, Map.Gmap.getZoom());
    $.each(markers, function(){ this.mgr = layer.mgr; });
    layer.empty = false;
    
    if (!layer.shown) {
      console.log('showing layer '+lname);
      layer.mgr && layer.mgr.show();
      layer.shown = true;
    }
  },
  ...
  open: function(marker, content) {
    ...
    // console.log("opening info window: " + marker + "---" + content);
    // console.log(this.open_marker);
    map.addOverlay(this.open_marker);
    console.log('attempting map open on');
    console.log(marker);
